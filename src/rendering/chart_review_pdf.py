...
{% macro render_severity_badge(severity) %}
  {% if severity and severity != "Low" %}
    <span class="severity-badge severity-{{ severity|lower }}">{{ severity }}</span>
  {% endif %}
{% endmacro %}

{% macro render_evidence(evidence_grade) %}
  {% if evidence_grade is defined and evidence_grade is mapping %}
    <div class="evidence-section">
      <h3>Evidence Grade
        <span class="grade-badge">GRADE: {{ evidence_grade["certainty"] }}</span>
      </h3>
      <p>{{ evidence_grade["rationale"] }}</p>
      {% if evidence_grade["sources"] %}
        <ul>
        {% for src in evidence_grade["sources"] %}
          <li {% if src.get("unverified") %}class="unverified" title="Citation not found in retrieved evidence"{% endif %}>
            {{ src["title"] }} ({{ src["source"] }}{% if src.get("year") %}, {{ src["year"] }}{% endif %}){% if src.get("unverified") %} *{% endif %}
          </li>
        {% endfor %}
        </ul>
      {% endif %}
...
{% macro render_plan(plan) %}
  ...
  {% if plan.problems is defined %}
    ...
    {% for problem in plan.problems %}
      <h2>{{ problem["Problem Name"] }}{{ render_severity_badge(problem.get("Severity")) }}</h2>
      {% for field, txt in problem.items() if field not in ["Problem Name", "Severity", "Evidence Grade"] %}
        <div class="row">
          <h3 {% if "Considerations" in field %}class="consideration"{% endif %}>{{ field }}</h3>
          <p>{{ txt.replace('\n','<br/>') }}</p>
        </div>
      {% endfor %}
      {{ render_evidence(problem.get("Evidence Grade")) }}
    {% endfor %}
  {% else %}
    {# v1 fallback: iterate dict keys #}
    {% for subkey, content in plan.items() %}
      {% if subkey.startswith("Problem") %}
        <h2>{{ content["Problem Name"] }}{{ render_severity_badge(content.get("Severity")) }}</h2>
        {% for field, txt in content.items() if field not in ["Problem Name", "Severity", "Evidence Grade"] %}
          <div class="row">
            <h3 {% if "Considerations" in field %}class="consideration"{% endif %}>{{ field }}</h3>
            <p>{{ txt.replace('\n','<br/>') }}</p>
          </div>
        {% endfor %}
        {{ render_evidence(content.get("Evidence Grade")) }}
      {% endif %}
    {% endfor %}
  {% endif %}
